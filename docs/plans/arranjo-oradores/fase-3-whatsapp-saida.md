# Fase 3 — WhatsApp de saída: templates na Meta primeiro, depois os envios
...
## Lição do v1 (a mais cara)

O v1 escreveu **12 templates e nunca submeteu nenhum à Meta**; o canal nunca operou.
Nesta fase a ordem é invertida: **item 1 é submeter e aprovar os templates reais**.
Nada de construir fluxo sobre template hipotético.
...
| `coordena_talk_reminder` | lembrete D-3 ao orador (disparo na fase 6; nasce aqui p/ aprovar junto) | mesmos |
...
- [ ] **Submeter à Meta e acompanhar até aprovar.** Registrar nome/idioma/categoria aprovados.
- [ ] Teste de definição (espelhar `TemplateDefinitionsTest` do v1): variáveis casam com o
  que os jobs enviam.

### 2. Envio de designação ao orador
...
- 4 templates **aprovados na Meta** (evidência real, não "escrito").
- Coordenador toca "Notificar" no celular → orador recebe o template com esboço e link;
  badge muda para "aguardando".
- Envio de convite por WhatsApp registra o send e entrega o link do portal.
