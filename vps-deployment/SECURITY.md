...
| Crítico | Reutilização de senha entre root/banco | ⚠️ Ação necessária |
...
**Problema:** Em `manifest.env`, `DB_ROOT_PASS` e `DB_PASSWORD` podem acabar com o mesmo valor. Uma exposição compromete todos os sistemas de uma vez.
...
```bash
# Na VPS, como root:
sudo -u postgres psql -c "ALTER USER coordena-v2_production PASSWORD 'nova-senha-forte';"

# Atualizar /opt/production/<PROJECT_NAME>/.env com a nova senha
# Reiniciar os containers:
docker compose -p coordena-v2-production restart
```
