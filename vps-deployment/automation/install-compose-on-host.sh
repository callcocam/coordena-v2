...
PROJECT_NAME="${PROJECT_NAME:-coordena-v2}"
...
if [[ ! -f "${_compose_src}" ]]; then
    _compose_src="${ROOT_DIR}/deployments/docker-compose.production.yml"
fi
...
if [[ "${START_SERVICES}" == "true" ]]; then
    ...
    echo "[app] Baixando imagem da aplicação coordena-v2-${APP_SLUG}..."
    echo "[app] (Se for o primeiro deploy a imagem pode ainda não existir no registry — tudo bem)"
    if (cd "${APP_DIR}" && docker compose -p "coordena-v2-${APP_SLUG}" pull 2>&1); then
        (cd "${APP_DIR}" && docker compose -p "coordena-v2-${APP_SLUG}" up -d)
        echo "[app] coordena-v2-${APP_SLUG} iniciado com sucesso"
    else
