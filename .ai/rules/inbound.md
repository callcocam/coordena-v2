...
## Sandbox envia botões como interactive.list_reply
O WhatsApp Cloud sandbox renderiza quick replies de template como `interactive.list_reply` (não `button.text` nem `interactive.button_reply`). Todo handler de botão deve ler o rótulo em `payload['button']['text'] ?? payload['interactive']['button_reply']['title'] ?? payload['interactive']['list_reply']['title']`. Ao criar novo handler de botão, copie o buttonText() de um existente e cubra os três formatos num teste Pest.
