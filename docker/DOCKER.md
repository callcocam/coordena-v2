...
> O nome do projeto é o token **`coordena-v2`** em todos os arquivos. O `init.sh` na raiz do scaffold
> substitui `coordena-v2` pelo nome real do seu projeto — rode-o antes de usar.
...
Rode `./init.sh` (substitui `coordena-v2` por todo lado), ou troque manualmente em `docker-compose.yml`:

| Superfície | Valor |
|---------|------------------|
| compose | `name: coordena-v2` |
| nginx   | `traefik.http.routers.coordena-v2-web.rule=Host(\`coordena-v2.localhost\`)` |
| pgadmin | `traefik.http.routers.coordena-v2-pgadmin.rule=Host(\`pgadmin.coordena-v2.localhost\`)` |
...
```env
APP_URL=http://coordena-v2.localhost
DB_CONNECTION=pgsql
DB_HOST=postgres
DB_PORT=5432
DB_DATABASE=coordena-v2
DB_USERNAME=coordena-v2
DB_PASSWORD=coordena-v2
REDIS_HOST=redis
REDIS_PORT=6379
MAIL_MAILER=smtp
MAIL_HOST=mailpit
