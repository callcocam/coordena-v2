...
<template>
    <aside class="sb-inspector">
        <div class="sb-tabs">
            <button :class="{ 'is-on': tab === 'wire' }" @click="tab = 'wire'">
                Rede
            </button>
            <button
                :class="{ 'is-on': tab === 'faults' }"
                @click="tab = 'faults'"
            >
                Falhas
            </button>
        ...
        <div v-if="tab === 'wire'" class="sb-pane">
            <p v-if="!message" class="sb-note">
                Clique numa bolha para ver o que realmente passou no fio: o
                envelope que enviamos, o webhook que voltou, a assinatura e quem
                escutou.
            </p>

            <template v-else>
                <div class="sb-kv">
                    <span>wamid</span><span>{{ message.wamid }}</span>
                </div>
                <div class="sb-kv">
                    <span>tipo</span><span>{{ message.type }}</span>
                </div>
                <div class="sb-kv">
                    <span>direção</span><span>{{ message.direction }}</span>
                </div>
                <div v-if="message.status" class="sb-kv">
                    <span>status</span><span>{{ message.status }}</span>
                </div>
                <div v-if="message.template" class="sb-kv">
                    <span>corpo do template</span>
                    <span>{{
                        message.template.source === 'definition'
                            ? 'arquivo local'
                            : 'Meta'
                    }}</span>
                </div>
                ...
                <template v-if="detail?.inbound_payload">
                    ...
                    <div class="sb-kv">
                        ...
                        <span
                            >{{
                                (detail.meta?.signature ?? '').slice(0, 24)
                            }}…</span
                        >
                    </div>
                    <div class="sb-kv">
                        <span>HTTP</span
                        ><span>{{ detail.meta?.status ?? '—' }}</span>
                    </div>
                </template>
                ...
                <template v-if="listeners.length">
                    ...
                    <pre>{{ listeners.join('\n') }}</pre>
                    <p class="sb-note">
                        Registro, não execução. Um listener
                        <code>ShouldQueue</code> roda noutro processo — o efeito
                        dele aparece depois, pelo polling.
                    </p>
                </template>
                ...
                <template v-if="out">
                    ...
                    <p class="sb-note">
                        Na vida real a Meta manda estes num webhook separado,
                        minutos depois. Aqui é um botão, pra você exercitar o
                        caminho de propósito.
                    </p>
                    <div class="sb-row">
                        <button
                            class="btn tiny"
                            @click="emit('status', { status: 'delivered' })"
                        >
                            Entregue
                        </button>
                        <button
                            class="btn tiny"
                            @click="emit('status', { status: 'read' })"
                        >
                            Lida
                        </button>
                        <button
        ...
        <div v-else class="sb-pane">
            ...
            <p class="sb-note">
                Fora dela, só template passa — texto livre volta como
                <strong>131047</strong>, que é terminal. É a surpresa nº 1 em
                produção, e a única forma de ensaiá-la era esperar um dia.
            </p>
            <div class="sb-row">
                <span
                    class="sb-pill"
                    :class="{ retry: conversation?.window_open }"
                >
            ...
            <p class="sb-note">
                Dispara uma vez e se desarma. O código de cada uma é o que a
                Meta devolveria de verdade — e <code>isTerminal()</code> aqui
                responde o mesmo que em produção.
            </p>

            <div v-for="fault in faults" :key="fault.key" class="sb-fault">
                <div class="sb-fault-main">
                    <div>{{ fault.title }}</div>
                    <div class="sb-fault-code">
                        {{ fault.code ?? 'sem código (rede)' }}
                    </div>
                </div>
                <span class="sb-pill" :class="{ retry: !fault.terminal }">
                    {{ fault.terminal ? 'terminal' : 'retentável' }}
                </span>
                <button class="btn tiny" @click="emit('arm', fault.key)">
                    Armar
                </button>
            </div>

            <p v-if="conversation?.armed_fault" class="sb-note">
                <strong>{{ conversation.armed_fault }}</strong> está armada — o
                próximo envio pra este contato vai falhar.
            </p>
        </div>
    </aside>
</template>
