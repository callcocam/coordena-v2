...
const panelUrl = computed(() =>
    new URL(page.url, window.location.origin).pathname.replace(/\/$/, ''),
);
...
const active = computed(
    () =>
        state.value.conversations.find((c) => c.id === state.value.selected) ??
        null,
);
...
async function poll() {
    ...
    const response = await fetch(
        `${panelUrl.value}/state${id ? `?conversation=${id}` : ''}`,
        {
...
const act = (path, data = {}) =>
    router.post(
        `${panelUrl.value}/${path}`,
        { conversation: state.value.selected, ...data },
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: poll,
...
return;
    ...
    const replyTo =
        inspected.value?.direction === 'outbound'
            ? inspected.value.wamid
            : null;
...
const template = computed(
    () => state.value.templates.find((t) => t.name === picked.value) ?? null,
);
...
function importChosen(all = false) {
    ...
    if (!all && waIds.length === 0) {
return;
}

    router.post(
        `${panelUrl.value}/participants/import`,
        { wa_ids: waIds },
        {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <div class="wa-sandbox">
        <header class="sb-top">
            ...
            <span class="sb-num">
                {{ business.display_phone_number }} · pnid
                {{ business.phone_number_id }}
            </span>
            <button class="btn tiny danger" @click="act('reset')">
                Limpar tudo
            </button>
        </header>
        ...
        <div class="sb-body">
            ...
            <nav class="sb-people">
                ...
                >
                    <span
                        class="sb-avatar"
                        :style="{ background: colorFor(conversation.wa_id) }"
                    >
                        {{ initials(conversation.name) }}
                    </span>
                    <span class="sb-person-main">
                        <span class="sb-person-name">{{
                            conversation.name
                        }}</span>
                        <span class="sb-person-sub">
                            <span
                                class="sb-dot"
                                :class="
                                    conversation.window_open ? 'open' : 'shut'
                                "
                            />
                            {{
                    ...
                    <span v-if="conversation.role" class="sb-role">{{
                        conversation.role
                    }}</span>
                </button>
                ...
                <div style="padding: 12px">
                    <template v-if="adding">
                        ...
                        />
                        <input
                            v-model="person.wa_id"
                            class="sb-field"
                            placeholder="Número (5548999999999)"
                        />
                        ...
                        <div class="sb-row">
                            ...
                            <button class="btn" @click="adding = false">
                                Cancelar
                            </button>
                        </div>
                    </template>
                ...
                <div
                    v-if="(state.recipients ?? []).length"
                    class="sb-recipients"
                >
                    <button
                        class="sb-recipients-toggle"
                    ...
                    <template v-if="importing">
                        ...
                        >
                            ...
                            >
                                ...
                                <span class="sb-rec-main">
                                    <span class="sb-rec-name">{{
                                        recipient.name
                                    }}</span>
                                    <span class="sb-rec-num">{{
                                        recipient.wa_id
                                    }}</span>
                                </span>
                                <span
                                    v-if="recipient.imported"
                                    class="sb-rec-tag"
                                    >já no chat</span
                                >
                            </label>
                        ...
                        <div class="sb-row" style="padding: 0 12px 12px">
                            <button
                                ...
                                @click="importChosen(false)"
                            >
                                Importar selecionados
                            </button>
                            <button
                                class="btn"
                                :disabled="!pendingRecipients.length"
                                @click="importChosen(true)"
                            >
                                Importar todos
                            </button>
                        </div>
            ...
            <main class="sb-chat">
                <template v-if="active">
                    <div class="sb-chat-top">
                        <span
                            class="sb-avatar"
                            :style="{ background: colorFor(active.wa_id) }"
                        >
                            {{ initials(active.name) }}
                        </span>
                        <span>
                            <span class="sb-chat-name">{{ active.name }}</span
                            ><br />
                    ...
                    <div ref="stream" class="sb-stream">
                        <p v-if="!state.messages.length" class="sb-empty">
                            Dispare um template pelo app — ou pelo seletor aqui
                            embaixo — e ele aparece nesta conversa. Depois
                            responda como {{ active.name }}: o webhook entra
                            pela rota real, assinado, e os listeners do app
                            rodam de verdade.
                        </p>

                        <template
                            v-for="{ message, day } in withDays"
                            :key="message.id"
                        >
                    ...
                    <footer class="sb-composer">
                        <div class="sb-as">
                            <button
                                :class="{ 'is-on': speaker === 'contact' }"
                                @click="speaker = 'contact'"
                            >
                                falar como {{ active.name }}
                            </button>
                            <button
                                :class="{ 'is-on': speaker === 'system' }"
                                @click="speaker = 'system'"
                            >
                        ...
                        <div class="sb-row" style="margin-top: 8px">
                            ...
                            >
                                <option :value="null">
                                    Disparar um template como o sistema…
                                </option>
                                <option
                        ...
                        <p v-if="!state.templates.length" class="sb-note">
                            Nenhum template encontrado. Aponte
                            <code>definitions_path</code> pra pasta dos arquivos
                            de definição — eles funcionam aqui antes de irem pra
                            Meta.
                        </p>
                    </footer>
                </template>
                ...
                <p v-else class="sb-empty">
                    Adicione um participante pra começar. Um cliente e um
                    responsável já bastam pra encenar o handoff inteiro.
                </p>
            </main>
