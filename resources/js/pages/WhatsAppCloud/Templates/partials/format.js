...
export function maxVar(text) {
    ...
    return m.reduce(
        (mx, tok) => Math.max(mx, parseInt(tok.replace(/\D/g, ''), 10) || 0),
        0,
    );
}
...
export function validateForm(f) {
    ...
    for (const v of f.bodyExamples) {
        if (/[\n\t]|\s{4,}/.test(v)) {
return `Exemplo "${v}" tem quebra de linha/tab — a Meta proíbe.`;
}
    }
    ...
    if (nHeader > 1) {
return 'O cabeçalho aceita no máximo 1 variável.';
}

    if (
        nHeader === 1 &&
        f.headerExamples.filter((v) => v.trim() !== '').length < 1
    ) {
    ...
    for (const b of f.buttons) {
        if (b.type === 'URL' && !b.url) {
return `Botão de URL "${b.text}" sem endereço.`;
}

        if (b.type === 'PHONE_NUMBER' && !b.phone_number) {
return `Botão de telefone "${b.text}" sem número.`;
}
    }
