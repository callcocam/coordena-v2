...
test('the team edit page can be rendered', function () {
    ...
    $response
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('members.0.cargos.0.key', DefaultCargo::Coordenador->value),
        );
});
...
test('deleting current team switches to alphabetically first remaining team', function () {
    ...
    $zuluTeam->members()->attach($user, ['is_owner' => true]);
    $user->assignCargo($zuluTeam, DefaultCargo::Coordenador->value);

    $alphaTeam = Team::factory()->create(['name' => 'Alpha Team']);
    $alphaTeam->members()->attach($user, ['is_owner' => true]);
    $user->assignCargo($alphaTeam, DefaultCargo::Coordenador->value);

    $betaTeam = Team::factory()->create(['name' => 'Beta Team']);
    $betaTeam->members()->attach($user, ['is_owner' => true]);
    $user->assignCargo($betaTeam, DefaultCargo::Coordenador->value);
...
test('leaving current team switches to alphabetically first remaining team', function () {
    ...
    $zuluTeam = Team::factory()->create(['name' => 'Zulu Team']);
    $zuluTeam->members()->attach($owner, ['is_owner' => true]);
    $zuluTeam->members()->attach($member, ['is_owner' => false]);

    $alphaTeam = Team::factory()->create(['name' => 'Alpha Team']);
    $alphaTeam->members()->attach($member, ['is_owner' => false]);

    $betaTeam = Team::factory()->create(['name' => 'Beta Team']);
    $betaTeam->members()->attach($member, ['is_owner' => false]);
